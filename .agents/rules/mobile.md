...
```
lib/
├── main.dart                              — ~20 lines: init + runApp only
├── app/
...
└── shared/
    ...
    └── widgets/
        ├── app_top_bar.dart               — AppTopBar widget
        ├── radial_gauge.dart              — RadialGauge widget (Syncfusion)
        ├── sensor_chart.dart              — SensorChart widget (fl_chart)
        ├── metric_card.dart               — MetricCard + SectionHeader widgets
...
- Consumed via singleton instance or passed down as constructor arg
...
| Setup / Offline wizard | 🟢 Refactored | Feature-first structure |
| Logs screen | 🟢 Refactored | Feature-first structure |
